---
name: doc-er
description: 'price-monitoring の docs/er.md（テーブル定義のみ）をメンテする。アソシエーション図は docs/association.md（doc-association スキル）が担当する。構成は ## テーブル名 / ### カラム / ### インデックス。事実は volumes/backend/db/schema.rb から転記 （編集しない）。テーブル追加・カラム変更・スキーマドキュメント、「er.md を更新して」 で使う。'
---
...
FK を追加・変更したときは、ユーザーに `docs/association.md` の更新も必要か確認する（`doc-association` スキル）。
