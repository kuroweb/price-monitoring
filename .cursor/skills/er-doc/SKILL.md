---
name: er-doc
description: 'price-monitoring の docs/er.md（テーブル定義のみ）をメンテする。アソシエーション図は docs/association.md を担当する別スキル。構成は ## テーブル名 / ### カラム / ### インデックス。事実は volumes/backend/db/schema.rb から転記（編集しない）。 テーブル追加・カラム変更・スキーマドキュメント、「er.md を更新して」で使う。'
---
