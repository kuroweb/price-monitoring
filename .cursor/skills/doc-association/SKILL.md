...
description: 'price-monitoring の docs/databases/association.md（テーブル間リレーション）をメンテする。 テーブル定義は docs/databases/er.md（doc-er スキル）が担当する。構成は ## 区分1 / ### 区分2（任意）/ Mermaid erDiagram（PK・FK と線のみ）。schema.rb を参照して整合 （編集しない）。アソシエーション図・ER 図・リレーション図、「association.md を更新して」 で使う。'
---
